...
export const DB = {
  async getDevice(device_id) {
    return await supabase.from('devices').select('*').eq('device_id', device_id).eq('revoked', false).single();
  },
  ...
  async getNotifications(user_id) {
    return await supabase.from('notifications').select('id, message, created_at').eq('user_id', user_id).eq('is_read', false).order('created_at', { ascending: false });
  },
  async addTrusted(row) {
    return await supabase.from('trusted_locations').insert(row).select('*').single();
  },
  async deleteTrusted(user_id, id) {
    return await supabase.from('trusted_locations').delete().eq('user_id', user_id).eq('id', id);
  },
  async listLocations(user_id, from_ms, to_ms) {
    let q = supabase.from('locations').select('lat, lon, ts_ms').eq('user_id', user_id);
